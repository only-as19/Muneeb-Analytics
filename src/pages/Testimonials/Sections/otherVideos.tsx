...
const OtherVideos:React.FC<VideoProps> = ({video}) => {
    console.log(video)
  return (
    <div>
      <div className="grid gap-y-4 md:grid-cols-2 gap-x-8">
        {video.map((video,index)=>(
           <button key={index} className="group cursor-pointer">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 bg-black bg-opacity-40 rounded-lg p-3 backdrop-blur-sm">
                      <div className="flex-shrink-0">
                        <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-blue-500">
                          <span className="text-base font-bold text-white">{video.clientName.charAt(0)}</span>
                        </div>
                      </div>
                      <div className="flex-1 min-w-0 text-left">
                        <p className="font-bold text-white text-sm">{video.clientName}</p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-gray-300 bg-black bg-opacity-40 px-2 py-1 rounded">{video.projectDuration}</span>
                      </div>
                    </div>
                  </div>
                </div>
