    ...
    <div className="grid gap-y-8">
      <div className="flex flex-col md:flex-row p-4  rounded-2xl shadow-xl gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <h2 className="text-lg font-semibold capitalize">Data systems for a nationwide pallet operation</h2>
          <p className="line-clamp-3 md:line-clamp-none  text-muted-foreground text-sm">
            {mainVideo.website && (
              <span className="italic">
                <Link to={mainVideo.website}>{mainVideo.company}</Link>
              </span>
            )}{":  "}
            {mainVideo.about}
          </p>
          <div  className="flex gap-2 flex-wrap">
            <span className="bg-secondary text-xs px-3 py-1.5 rounded-3xl shadow">My role: {mainVideo.myRole}</span>
            <span className="bg-secondary text-xs px-3 py-1.5 rounded-3xl shadow">{mainVideo.projectDuration}</span>
      ...
      <div className="grid md:grid-cols-2 md:gap-x-4">
        ...
        <div className="flex flex-col gap-1 h-full">
          ...
          <h2 className="text-lg font-semibold capitalize">{video.myRole}</h2>
          <p className="line-clamp-3 md:line-clamp-none  text-muted-foreground text-sm">
            {video.website && (
              <span className="italic">
                <Link to={video.website}>{video.company}:</Link>
              </span>
            )}{" "}
            {video.about}
          </p>
          <div className=" mt-auto flex gap-2 flex-wrap">
            <span className="bg-secondary text-xs px-3 py-1.5 rounded-3xl shadow">My role: {video.myRole}</span>
            <span className="bg-secondary text-xs px-3 py-1.5 rounded-3xl shadow">{video.projectDuration}</span>
            <div className="bg-secondary text-xs px-3 py-1.5 rounded-3xl shadow flex items-center gap-1">
            <FaStar strokeWidth={1} color="orange" size={14}/>
